...
@endif
@if (session('error'))
    <div class="alert alert-danger mt-2 shadow-sm" id="error-alert">
        {{ session('error') }}
    </div>
@endif
    ...
    <div class="card-body">
        ...
        <div class="card shadow mb-4 bg-white px-3">
            <div class="card-header py-3 d-flex justify-content-between align-items-center">
                <form action="{{ route('admin.guru.index') }}" method="GET" class="form-inline">
                    <input type="text" name="search" value="{{ request('search') }}" class="form-control form-control-sm mr-2" placeholder="Cari nama atau NIP">
                    <button type="submit" class="btn btn-sm btn-primary">Cari</button>
                </form>
                <form action="{{ route('admin.guru.destroyAll') }}" method="POST" onsubmit="return confirm('Yakin hapus semua data guru?')">
                    @csrf
                    @method('DELETE')
            ...
            <div class="table-responsive" style="max-height: 400px; overflow-y: auto;">
                <table class="table table-bordered table-sm" width="100%" cellspacing="0">
                    ...
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>{{ $g->nip }}</td>
                            <td>
                                <a href="{{ route('admin.guru.edit', ['guru' => $g->id_guru]) }}" class="btn btn-sm btn-warning">Edit</a>
                                <form action="{{ route('admin.guru.destroy', ['guru' => $g->id_guru]) }}" method="POST" class="d-inline">
              ...
              {{-- Import Data Siswa --}}
                  <div class="my-3">
                      ...
                      <form action="{{ route('admin.guru.import') }}" method="POST" enctype="multipart/form-data" class="d-flex flex-wrap align-items-center">
                      @csrf
                      ...
                      <a href="{{ route('admin.guru.template') }}" class="btn btn-outline-success btn-sm mr-2 mb-2">
                          <i class="fas fa-download"></i> Download Template
                      </a>
                      <small class="text-muted mb-2">
                          📌 Gunakan file Excel (<code>.xls</code> / <code>.xlsx</code>)
                          Kolom: <strong>Nama Guru | NIP |</strong>.
